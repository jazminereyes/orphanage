                        ...
                        <h3>Statement of Expense</h3>
                            <table class="table table-striped">
                                ...
                                <?php 
                                    ...
                                    $sql = "SELECT * FROM scholar JOIN s_budget as bud on scholar.scholarID = bud.scholarID JOIN s_expensestatement as exp on bud.budgetID = exp.budgetID WHERE scholar.scholarID = '$scholarID'";
                                    $sv = mysqli_query($con, $sql);

                                    while($res = mysqli_fetch_array($sv)){
                                        $cat = $res['expenseCategory'];
                                        $rem = $res['amountRemaining'];

                                        if($cat == "supplies"){
                                            $cat = "School Supplies";
                                            $rem = $res['equivSupplies'];
                                        }
                                        elseif($cat == "uniform"){
                                            $cat = "School Uniform";
                                            $rem = $res['equivUniform'];
                                        } 
                                        elseif($cat == "contributions"){
                                            $cat = "School Contribution";
                                            $rem = $res['equivContrib'];
                                        }
                                        elseif($cat == "transportation"){
                                            $cat = "Transportation";
                                            $rem = $res['equivTranspo'];
                                        }
                                        elseif($cat == "projects"){
                                            $cat = "School Projects";
                                            $rem = $res['equivProjects'];
                                        }
