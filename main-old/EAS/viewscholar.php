...
<script src="backend/updatescholar.js"></script>
            ...
            <div class="card">
              <div class="card-header p-2">
                <ul class="nav nav-pills">
                  ...
                  <li class="nav-item"><a class="nav-link" href="#timeline" data-toggle="tab" >Medical Information</a></li>
                  <li class="nav-item"><a class="nav-link" href="#settings" data-toggle="tab" >Family Background</a></li>
                  <li class="nav-item"><button type='button' class='btn btn-success btn-flat' style="margin-left: 180px;" onclick="updateInfo()">
              ...
              <div class="card-body">
                <div class="tab-content">
                  ...
                  <form method="post" action="backend/updatesch.php" enctype="multipart/form-data">
                      ...
                      echo '<input type="text" class="form-control input req" id="fn" name="first" style="display: none" value="'.$rec['firstName'].'"/>
                      </div>';
                ...
                <option value="SHS_G12">Senior High School - Grade 12</option>
                  ...
                  $sql = "SELECT * FROM s_medicalreport WHERE scholarID = '$scholar_id' ORDER BY medicalID DESC LIMIT 1";
                  ...
                  if(mysqli_num_rows($qry) == 0){
                    ...
                    $weight = $info['weight'];
                    $height = $info['height'];
                    $stat = $info['weightStatus'];
                    $dm = $info['discolorMarks'];
                    $hc = $info['hairColor'];
                    $ec = $info['eyeColor'];
                    $ld = $info['lastDateHospitalized'];
                    $lp = $info['lastPlaceHospitalized'];
                    $appid = $info['scholarAppID'];
                    $hid = $info['healthInfoID'];

                  }
                  else{

                    $med = mysqli_fetch_array($qry);
                    $weight = $med['weight'];
                    $height = $med['height'];
                    $stat = $med['remarks'];

                    $sel = "SELECT s.scholarAppID, med.healthInfoID, discolorMarks, hairColor, eyeColor, lastDateHospitalized, lastPlaceHospitalized FROM s_appform as app JOIN scholar as s ON app.scholarAppID = s.scholarAppID JOIN s_healthinfo as med ON app.healthInfoID = med.healthInfoID WHERE scholarID = $scholar_id";
                    $get = mysqli_query($con, $sel);
                    $info = mysqli_fetch_array($get);
                    ...
                    $hid = $info['healthInfoID'];
                      ...
                      echo '<input type="hidden" name="hid" value="'.$appid.'" />';
                      echo '<input type="hidden" name="sid" value="'.$scholar_id.'" />';

                      echo '<label id="wt" class="lbl">Weight</label>';
                      echo '<input type="text" class="form-control" id="wgt" class="or" name="weight" value="'.$weight.' kg" style="width: 20%;" disabled>';
                      echo '<label id="st" class="lbl">Weight Status</label>';
                      echo '<input type="text" class="form-control" id="ws" class="or" name="stat" value="'.$stat.'" style="width: 30%;" disabled><br/>';
                      echo '<label id="ht" class="lbl">Height</label>';
                      echo '<input type="text" class="form-control" id="hgt" class="or" name="height" value="'.$height.' cm" style="width: 20%;" disabled>';
                      echo '<label id="dm" class="lbl">Distinguishing Marks</label>';
                      echo '<input type="text" class="form-control" id="mark" class="or" name="marks" value="'.$dm.'" style="width: 20%;" disabled><br/>';
                      echo '<label id="hc" class="lbl">Hair Color</label>';
                      echo '<input type="text" class="form-control" id="hclr" class="or" name="hair" value="'.$hc.'" style="width: 30%;" disabled>';
                      echo '<label id="ec" class="lbl">Eye Color</label>';
                      echo '<input type="text" class="form-control" id="eclr" class="or" name="eye" value="'.$ec.'" style="width: 30%;" disabled><br/>';
                      echo '<label id="ad" class="lbl">Last Date Hospitalized</label>';
                      echo '<input type="text" class="form-control" id="ldh" class="or" name="placehospi" value="'.$ld.'" style="width: 30%;" disabled><br/>';
                      echo '<label id="bd" class="lbl">Last Place Hospitalized</label>';
                      echo '<input type="text" class="form-control" id="lph" class="or" name="datehospi" value="'.$lp.'" style="width: 50%;" disabled><br/>';
                  ...
                  while($fam = mysqli_fetch_array($my)){
                    ...
                    if($fam['relationType'] == "Mother"){
                      echo "<div class='col-6'>
                        ...
                        <div class='col-sm-10'>
                          <input type='number' class='form-control' id='mcno' name='mcno' value='".$fam['contactNo']."' disabled>
                        </div>
                        </div>";
                    ...
                    elseif($fam['relationType'] == "Father"){
                      ...
                      <label for='fname' class='col-sm-5 control-label lbl'>Father's Name</label>
                        ...
                        <div class='col-sm-10'>
                          <input type='number' class='form-control' id='fcno' name='fcno' value='".$fam['contactNo']."' disabled>
                        </div>
                        </div>";
                        $fflag = 1;
                    }
                  ...
                  if($mflag == 1 && $fflag == 0){
                      ...
                      <label for='fname' class='col-sm-5 control-label lbl'>Father's Name</label>
                        ...
                        <label for='fcno' class='col-sm-5 control-label lbl'>Contact No.</label>
                        <div class='col-sm-10'>
                          <input type='number' class='form-control' id='fcno' name='fcno' disabled>
                        </div>
                        </div>";
                    }
                    elseif($fflag == 1 && $mflag == 0){
                     ...
                     <label for='mname' class='col-sm-5 control-label lbl'>Mother's Name</label>
                        ...
                        <label for='mcno' class='col-sm-5 control-label lbl'>Contact No.</label>
                        <div class='col-sm-10'>
                          <input type='number' class='form-control' id='mcno' name='mcno' disabled>
                        </div>
                        </div>";
                    }
                  ?> 
                  ...
                  <!-- /.row -->
                  <button type="submit" name="submit" class="btn btn-primary float-right" id="savebtn" style="display: none;" onclick="saveInput(); validateInput();">SAVE</button>
                  </form>
