...
<script type="text/javascript">
  function inputAmount(str){

    if(str == "School Supplies"){
      document.getElementById('rowName').value = "School Supplies";
    }
    else if(str == "School Projects"){
      document.getElementById('rowName').value = "School Project";
    }
    else if(str == "School Uniform"){
      document.getElementById('rowName').value = "School Uniform";
    }
    else if(str == "School Contribution"){
      document.getElementById('rowName').value = "School Contribution";
    }
    else if(str == "Transportation"){
      document.getElementById('rowName').value = "Transportation";
    }




    document.getElementById('displayAmt').style = "inline-block";
  }
  function getColumnName(str){
    if(str == 1){
      document.getElementById('columnName').value="School Supplies";
    }
    else if(str == 2){
      document.getElementById('columnName').value="School Projects";
    }
    else if(str == 5){
      document.getElementById('columnName').value="Transportation";
    }
    else if(str == 4){
      document.getElementById('columnName').value="School Contribution";
    }
    else if(str == 3){
      document.getElementById('columnName').value="School Uniform";
    }
  }
</script>
  <!-- SCRIPTS -->
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      ?>
        ...
        <th width="35%" colspan="3" style="border-bottom: none">Total Amount</th>
      ...
      <?php
        ...
        // $columnid = array("1" => $get['equivTranspo'], "2" =>  $get['equivContrib'], "3" => $get['equivUniform'], "4" => $get['equivProjects'], "5" => $get['equivSupplies'])
        ...
        $pctsupp = $row[0] / $e_supply * 100;
        $pctproj = $row[1] / $e_proj * 100;
        $pctuni = $row[2] / $e_unif * 100;
        $pctcont = $row[3] / $e_cont * 100;
        $pcttrans = $row[4] / $e_transpo * 100;

        if ($pctsupp<=20)
        {
          $disp = '<td><button type="button" class="btn btn-danger" data-toggle="modal" data-target="#transfer" onclick="getColumnName(1)">Transfer From Budget</button></td>';
        }
        else
        {
          $disp = '<td></td>';
        ...
        if ($pctproj<=20)
        {
          $disp2 = '<td><button type="button" class="btn btn-danger" data-toggle="modal" data-target="#transfer" onclick="getColumnName(2)" >Transfer From Budget</button></td>';
        }
        else
        {
          $disp2 = '<td></td>';
        ...
        {
          $var = "unif";
          $disp3 = '<td><button type="button" class="btn btn-danger" data-toggle="modal" data-target="#transfer" onclick="getColumnName(3)">Transfer From Budget</button></td>';
        }
        else
        {
          $disp3 = '<td></td>';
        ...
        if ($pctcont<=20)
        {
          $disp4 = '<td><button type="button" class="btn btn-danger" data-toggle="modal" data-target="#transfer" onclick="getColumnName(4)">Transfer From Budget</button></td>';
        }
        else
        {
          $disp4 = '<td></td>';
        ...
        <td>'.$contrib.'</td>
        <td>'.$row[3].'</td>
        <td>'.$contribcr.'</td>'.$disp4.'
        </tr>';

        if ($pcttrans<=20)
        {
          $disp5 = '<td><button type="button" class="btn btn-danger" data-toggle="modal" data-target="#transfer" onclick="getColumnName(5)">Transfer From Budget</button></td>';
        }
        else
        {
          $disp5 = '<td></td>';
      ...
      <div class="modal-dialog">
        ...
        <div class="modal-body">
          <div align="right">
            ...
            <br/><h5><strong>Recommended Budget Allocation</strong></h5><br/>

            <form method="post" action="backend/transferamt.php">
            ...
            $sel = "SELECT equivTranspo, equivContrib, equivUniform, equivProjects, equivSupplies FROM s_budget WHERE scholarID = '$scholar_id'";
            $qry = mysqli_query($con, $sel);
            $get = mysqli_fetch_array($qry);

            $budget = array("Transportation" => $get['equivTranspo'], "School Contribution" =>  $get['equivContrib'], "School Uniform" => $get['equivUniform'], "School Projects" => $get['equivProjects'], "School Supplies" => $get['equivSupplies']);
            // $rowid = array(0 => $get['equivTranspo'], 1 =>  $get['equivContrib'], 2 => $get['equivUniform'], 3 => $get['equivProjects'], 4 => $get['equivSupplies']);
            ...
            foreach($budget as $key => $val){
              ...
              echo "<td>".$key."</td>";
              echo "<td>".$val."</td>";
              echo "<td><button id='t_btn' type='button' class='btn btn-primary' onclick='inputAmount(this.value)' data-btn='".$key."' value='".$key."' >Transfer</button></td>";
              echo "<tr>";
              if($i++ > 0) break;
            }
          ...
          <div id="displayAmt" class="row" style="display: none">
            <h5 class="text-center">Transferring budget</h5>
            <div class="row">
              <div class="col-md-6">
                <label>From:</label>
                <input type="text" class="form-control input" name="rowName" id="rowName" readonly />
              </div>
              <div class="col-md-6">
                <label>To</label>
                <input type="text" class="form-control input" name="columnName" id="columnName" readonly />
              </div>
            </div>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-outline pull-left" data-dismiss="modal">Close</button>
          <input type="submit" name="submit" class="btn btn-success pull-left" value="Confirm" />
        </form>
      </div>
    </div>
            <!-- /.modal-content -->
